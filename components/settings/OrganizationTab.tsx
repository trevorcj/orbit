...
import { Copy, Check, Loader2, Building2 } from "lucide-react";
...
export default function OrganizationTab() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12 bg-white rounded-xl border border-zinc-100">
  ...
  return (
    <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-8 rounded-xl border border-zinc-100 bg-white">
      {/* Left Form Column */}
      <div className="lg:col-span-2 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-5">
          ...
          <Input
            ...
            readOnly
            value={orgSlugUrl}
            className="border-zinc-200 bg-zinc-50/50 pr-12 !text-zinc-500 font-mono text-xs">
            <button
              type="button"
              onClick={handleCopy}
              className="p-1.5 rounded hover:bg-zinc-100 text-zinc-400 hover:text-zinc-600 transition-colors pointer-events-auto cursor-pointer">
              {copied ? (
      ...
      <div className="flex flex-col items-center lg:items-start gap-4 lg:pl-8 lg:border-l border-zinc-100">
        ...
        {org?.logo_url ? (
          <div className="relative w-24 h-24 rounded-2xl overflow-hidden border border-zinc-200 shadow-xs">
            <Image
              src={org.logo_url}
              alt={org.name}
              fill
